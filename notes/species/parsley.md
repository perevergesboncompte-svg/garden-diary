# Parsley

- Botanical name: Petroselinum crispum [EcoCrop]
- Family: Umbelliferae [EcoCrop]

## Sowing
- Sowing depth mm: 6 [hand-entered]
- Row spacing cm: 30 [OpenFarm]
- Plant spacing cm: 10 [OpenFarm]
- Days to germination: 14 [PlantVarietyDB, median of 2 varieties]
- Germination window days: 14 to 28 [openplantdb, median of 12 varieties, matches the hand-entered estimate]
- Germination soil temperature F: 50 to 70 [openplantdb, median of 12 varieties]
- Local sowing months: October to March [UC Master Gardeners Orange County]

## Growing
- Days to harvest from seed: 70 to 90 [openplantdb, median of 11 varieties sown direct, not transplanted]
- Field cycle days: 70 to 365 [EcoCrop, whole-field cycle, longer than days to first pick]
- Optimal temperature F: 52 to 68 [EcoCrop]
- Absolute temperature F: 45 to 82 [EcoCrop]
- Bolts above F: 85 [hand-entered]
- Killed below F: 32 [EcoCrop]
- Target soil pH: 6.5, lime below 6.0 [Mid-Atlantic Commercial Vegetable Recommendations. In a high-organic raised-bed mix above 6 percent organic matter, crops wanting 6.5 tolerate about 5.6]
- Soil pH: 5.3 to 8.3, optimal 6 to 7.5 [EcoCrop]
- Crop coefficient: 0.9 [hand-entered, small-vegetable class after FAO-56 Table 12, cited not copied]

## Raised beds and containers
- Root depth: shallow [INRAE]
- Container depth inches: 8 [Maryland 8]
- Raised bed: fits at 8 inches despite the taproot [hand-entered]
- Container friendly: 2 of 2 varieties say yes [PlantVarietyDB]
- Soil profile depth: shallow (20-50 cm) [EcoCrop, field soil depth class, NOT rooting depth, do not read as a pot size]

## Soil and rotation
- Nutrient demand: moderate [INRAE]
- Rotation gap under cover years: 3 [INRAE]
- Rotation gap in open ground years: 4 [INRAE]
- Do not follow: Apiaceae [INRAE]

## Bolting
- Cold is the main trigger: 32 to 50 F, best 35 to 43 F for 5 to 8 weeks vernalises this crop and it then runs to seed [MSU Extension]
- Nothing reverses it once vernalised [MSU Extension]

## Harvest
- Repeat harvest: outer stalks, 10 to 12 months [hand-entered]

## Companions
- Keep away from: lettuce [powerplant]
- Other pairings recorded: 9, all for crops outside this library [powerplant]
